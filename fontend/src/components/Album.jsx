...
import "./hoe.css";
export default function Album({togleLogin,addAlbum}){
    ...
    useEffect(() => {
        async function getalbum(){
            ...
            try {
                ...
                let album=await axios.get(`http://localhost:2345/music/allAllbum?page=${page}&limit=${limit}&year=${year?"-1":"1"}`)
                    if(album!==undefined){
        ...
        getalbum()
    },[page,year,forceUpade,addAlbum])
    return (
        <>
        <div className="contest">
            {albumdata!==undefined?<div>
                &nbsp;  
                <Button 
                onClick={()=>{
                    setyear(!year)
                }} style={{margin:"1% 0%"}} variant="outlined">Sort year</Button>
                </div>:""}
                {load1? <p>Loading...</p> : error1? <p>Something went wrong</p>:
                <div>
                {albumdata!==undefined?albumdata?.map((e)=>(
                    <AlbumE e={e} togleLogin={togleLogin} key={e._id} setforceUpade={setforceUpade}
                    forceUpade={forceUpade}
                        />
                )):"No Album is available"}</div>
                }
